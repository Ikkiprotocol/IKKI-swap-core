...
// File @IKKI-finance/IKKI-swap-core/contracts/interfaces/IIKKIPair.sol@v0.1.0
...
// File @IKKI-finance/IKKI-swap-core/contracts/interfaces/IIKKIFactory.sol@v0.1.0
...
// File @IKKI-finance/IKKI-swap-periphery/contracts/interfaces/IIKKIRouter01.sol@v1.1.0-beta.0
...
// File @IKKI-finance/IKKI-swap-periphery/contracts/interfaces/IIKKIRouter02.sol@v1.1.0-beta.0
...
// File @IKKI-finance/IKKI-swap-periphery/contracts/interfaces/IWCRO.sol@v1.1.0-beta.0
...
contract IKKIZap is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    /* ========== CONSTANT VARIABLES ========== */

    address public immutable WCRO;
    IIKKIRouter02 public immutable ROUTER;
    IIKKIFactory public immutable FACTORY;
    ...
    constructor(address _wcro, address _router) {
        WCRO = _wcro;
        ROUTER = IIKKIRouter02(_router);
        FACTORY = IIKKIFactory(ROUTER.factory());
    ...
    function zapInToken(
        ...
        require(isToken(_fromToken), "IKKIZap:zapInToken: given fromToken is not token");
        require(isToken(_toTokenOrLp) || isLP(_toTokenOrLp), "IKKIZap:zapInToken: given toTokenOrLp is not token or LP");
        require(_inputAmount > 0, "IKKIZap:zapInToken: given amount should > 0");
        ...
        require(outputAmount >= _outputAmountMin, "IKKIZap:zapInToken: output amount less than expected");
    ...
    function zapIn(address _toTokenOrLp, uint256 _outputAmountMin) external payable nonReentrant returns (uint256) {
        require(msg.value > 0, "IKKIZap:zapIn: given amount should > 0");
        IWCRO(WCRO).deposit{value: msg.value}();
        require(isToken(_toTokenOrLp) || isLP(_toTokenOrLp), "IKKIZap:zapIn: given toTokenOrLp is not token or LP");
        uint256 outputAmount = _zapInFromToken(WCRO, msg.value, _toTokenOrLp, msg.sender);
        require(outputAmount >= _outputAmountMin, "IKKIZap:zapIn: output amount less than expected");
    ...
    function zapOut(
        ...
        require(isLP(_fromLp), "IKKIZap:zapOut: should zap out from LP Address");
        require(_fromLp != _toTokenOrLp, "IKKIZap:zapOut: input = output");
        require(_inputAmount > 0, "IKKIZap:zapOut: given amount should > 0");
        ...
        if (isLP(_toTokenOrLp)) {
            ...
            revert("IKKIZap:zapOut: should zap out to Token or LP Address");
        }
    ...
    function fetchLiquidityPoolsFromFactoryWithIndex(uint256 _startFromPairIndex, uint256 _interval) public {
        ...
        require(
            ...
            "IKKIZap:fetchLiquidityPoolsFromFactoryWithIndex: _startFromPairIndex should < factoryPairLength"
        );
    ...
    function _removeLiquidityToToken(
        ...
        require(isLP(_lp), "IKKIZap:_removeLiquidityToToken: _lp is Non LP Address");
        IIKKIPair pair = IIKKIPair(_lp);
    ...
    function _swapTokenToLP(
        ...
        require(isLP(_lp), "IKKIZap:_swapTokenToLP: _lp is Non LP Address");
    ...
    function _swapTokenToToken(
        ...
        require(amounts[amounts.length - 1] > 0, "IKKIZap:_swapTokenToToken: output amounts invalid - 0 amoount");
    ...
    function _getPathForTokenToToken(address _fromToken, address _toToken) private view returns (address[] memory) {
        address[] memory path;
        require(_fromToken != _toToken, "IKKIZap:_swapTokenToToken: Not Allow fromToken == toToken");
        ...
        require(path.length > 0, "IKKIZap:_getPathForTokenToToken: Does not support this route");
        return path;
    }

    function _getSuitableIntermediateToken(address _fromToken, address _toLp) private view returns (address) {
        IIKKIPair pair = IIKKIPair(_toLp);
        address token0 = pair.token0();
        address token1 = pair.token1();
        ...
        revert("IKKIZap:_getSuitableIntermediateToken: Does not support this route");
    }
    ...
    function addToken(address _tokenAddress) external onlyOwner {
        require(tokens[_tokenAddress] == 0, "IKKIZap:addToken: _tokenAddress is already in token list");
    ...
    function _addToken(address _tokenAddress, bool _isFromFactory) private {
        ...
        require(isLP(_tokenAddress) == false, "IKKIZap:_addToken: _tokenAddress is LP");
    ...
    function addIntermediateToken(address _tokenAddress) public onlyOwner {
        require(
            ...
            "IKKIZap:addIntermediateToken: _tokenAddress is already in token list"
        );
    ...
    function _addIntermediateToken(address _tokenAddress) private {
        ...
        require(isLP(_tokenAddress) == false, "IKKIZap:_addIntermediateToken: _tokenAddress is LP");
    ...
    function _addLiquidityPool(address _lpAddress, bool _isFromFactory) private {
        ...
        if (!liquidityPools[_lpAddress]) {
            IIKKIPair pair = IIKKIPair(_lpAddress);
            address token0 = pair.token0();
            address token1 = pair.token1();
            if (!isToken(token0)) {
